...
export default function BuyTickets({ submissions }) {
  ...
  const handleBuyTicket = async (event) => {
    ...
    const payment = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
      suggestedParams: await algod.getTransactionParams().do(),
      amount: (((Number(event.ticketPrice)) * ((100 - event.royalty_percentage_to_artist) / 100)) * 1000000) + 500_000, // 500_000 is box mbr, later on calculate it for now just taking it enough big number to save some time
      from: sender.addr,
      to: algosdk.getApplicationAddress(event.appID),
    });

    const royalty_payment = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
      suggestedParams: await algod.getTransactionParams().do(),
      amount: (((Number(event.ticketPrice)) * ((event.royalty_percentage_to_artist) / 100)) * 1000000),
      from: sender.addr,
      to: event.royalty_receiver_address,
    });
    const args = [event.asset_ID, event.organizer_address, event.ticket_id, Number(event.ticketPrice) * 1000000, { txn: payment, signer: sender.signer }, { txn: optin, signer: sender.signer }, { txn: royalty_payment, signer: sender.signer }];
          ...
          <div className="col-md-4" key={index} style={{ margin: '30px 0' }}>
            ...
            >
              ...
              <div className="card-body">
                ...
                <p className="card-text">
                  royalty_receiver_address: {event.royalty_receiver_address}
                </p>
                <p className="card-text">
                  royalty_percentage_to_artist: {event.royalty_percentage_to_artist}
                </p>
                <button
